...
返すJSON:
{
 ...
 "farm_id": "a"|"b"|null,
 "items": [ {"item":"正式な餌名","units":[{"unit":"単位名","n":数値}]} ],
...
- 1つの餌に対して複数行で内訳が続く場合（例「50コは3パレット / 35コは2パレット」）は、その餌の同じ単位を合算して1つにまとめる。数量として妥当な数値だけを拾い、意味の取れない断片は無視してよい。
- 単位名は必ずその餌が持つ単位ラベルか「kg」を使う。
- 餌名は正式名に正規化（別名・誤字も。例「いてそだ」「SODA」→「重曹」、「サイレージ」→「ベトナムｃｓ」、「シボウサン」→「脂肪酸カルシウム」）。
